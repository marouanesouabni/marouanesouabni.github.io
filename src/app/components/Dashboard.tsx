...
export function Dashboard() {
  ...
  return <main className="min-h-screen bg-[#0c0c0c] text-bone px-6 py-8 sm:px-10 lg:px-14">
    <div className="max-w-6xl mx-auto">
      <header className="flex items-start justify-between gap-6 border-b border-bone/10 pb-7 mb-8">
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-8">
        ...
        <Metric label="NEEDS REVIEW" value={unread} accent />
        <Metric label="DATABASE" value={databaseConnected ? "CONNECTED" : "OFFLINE"} />
      </div>
      ...
      <section className="border border-bone/10">
        ...
        {messages.length === 0 && !status ? <p className="p-8 text-sm text-bone/50">No messages yet.</p> : messages.map(message => <article key={message._id} className="p-5 border-b last:border-b-0 border-bone/10">
...
function Metric({ label, value, accent = false }: { label: string; value: string | number; accent?: boolean }) {
  return <div className="border border-bone/10 p-5"><p className="text-[10px] tracking-[0.2em] text-bone/45 mb-3">{label}</p><p className={`text-3xl ${accent ? "text-amber" : "text-bone"}`}>{value}</p></div>;
}
